...
class Cellori:
    ...
    def __init__(self,image,**kwargs):
        ...
        watershed = sitk.MorphologicalWatershedFromMarkers(sitk.GetImageFromArray(np.zeros(self.image.shape)),sitk.GetImageFromArray(markers),markWatershedLine=False)
    ...
    def segment(self,segmentation_mode='combined',threshold_locality=0.5,sigma=None,nuclei_diameter=None,coordinate_format='indices'):
        ...
        ----------
            segmentation_mode : {'combined', 'intensity', 'morphology'}, optional, default 'combined'
                ...
                * ‘morphology’: Use an morphology-only maxima metric.
            threshold_locality : float, optional, default 0.5
                Fractional weight on local intensity used in thresholding. The value must be between 0 (global thresholding) and 1 (local thresholding).
            sigma : float, optional, default 1.5
                Gaussian sigma used for denoising. If ``None``, the value will be determined by automatic parameter detection.
        ...
        masks,coords = self._segment(self.image,self.watershed_labeled,segmentation_mode,threshold_locality,sigma,nuclei_diameter)
    ...
    def _segment(self,image,watershed_labeled,segmentation_mode,threshold_locality,sigma,nuclei_diameter,origin=None):
        ...
        coords,binary = self._find_nuclei(image,watershed_labeled,segmentation_mode,sigma,threshold_locality,nuclei_diameter,origin)
        masks = self._get_masks(binary,coords)
        masks,coords = self._merge_correct(masks)

        return masks,coords

    def _find_nuclei(self,image,watershed_labeled,segmentation_mode,sigma,threshold_locality,nuclei_diameter,origin):
        ...
        for region in watershed_regions:
            ...
            global_binary_crop = self.global_binary[indices[0] + origin[0]:indices[1] + origin[0],indices[2] + origin[1]:indices[3] + origin[1]]

            binary_crop = self._conditional_local_threshold(image_crop,region.image,global_binary_crop,block_size,threshold_locality,self.background_std)
        ...
        for region in regions:
            ...
            if segmentation_mode == 'combined':
                ...
                binary_distance = self.normalize(binary_distance)
                image_crop = self.normalize(image_crop)
            ...
            maxima = feature.peak_local_max(metric,min_distance=round(nuclei_diameter / 4),threshold_rel=0.5,exclude_border=False)
    ...
    def _merge_correct(self,masks):
        ...
        if len(cells) > 0:
            ...
            for cell in cells:
                ...
                merge_candidates = [merge_candidate for merge_candidate in idx[cell] if merge_candidate not in merged_cells]
                ...
                cell_indices = np.array([cell_region.bbox[0],cell_region.bbox[2],cell_region.bbox[1],cell_region.bbox[3]])
                cell_crop = masks[cell_indices[0]:cell_indices[1],cell_indices[2]:cell_indices[3]]
                cell_crop = np.where(cell_crop == cell,cell_crop,0)

                for merge_candidate in merge_candidates:

                    merge_candidate_region = regions[merge_candidate - 1]
                    merge_candidate_indices = np.array([merge_candidate_region.bbox[0],merge_candidate_region.bbox[2],merge_candidate_region.bbox[1],merge_candidate_region.bbox[3]])
                    merge_candidate_crop = masks[merge_candidate_indices[0]:merge_candidate_indices[1],merge_candidate_indices[2]:merge_candidate_indices[3]]
                    merge_candidate_crop = np.where(merge_candidate_crop == merge_candidate,merge_candidate_crop,0)

                    merge_indices = np.array([min(cell_indices[0],merge_candidate_indices[0]),max(cell_indices[1],merge_candidate_indices[1]),min(cell_indices[2],merge_candidate_indices[2]),max(cell_indices[3],merge_candidate_indices[3])])
                    ...
                    if (4 * cell_region.area < self.min_area) | (4 * merge_candidate_region.area < self.min_area) | ((merge_region.area < 4 * self.min_area) & (merge_region.solidity > 0.975 * average_solidity)):
                        new_cell = min(cell_map[cell],cell_map[merge_candidate])
                        corrected_masks[merge_indices[0]:merge_indices[1],merge_indices[2]:merge_indices[3]][merge_test > 0] = new_cell
    ...
    def _conditional_local_threshold(self,image,mask,global_binary,block_size,k_max,c):
        ...
        ----------
            ...
            k_max : float
                Maximum fractional weight on local intensity used in thresholding. The value must be between 0 (global thresholding) and 1 (local thresholding).
            c : float
        ...
        -------
            threshold : numpy.ndarray
                Array of local threshold values. All pixels in the input image higher than the corresponding pixel in the threshold array are considered foreground.
        """
